...
class Trainer:
    ...
    def __init__(self, args, ngpus_per_node, wandb_logger=None):
        ### hmt
    ...
    def train_epoch(self, epoch):
        ...
        for i, batch_dict in enumerate(self.train_loader):
            ...
            ### hmt
            def get_self_cl(features):
                s = features @ features.T
                s = s - torch.eye(s.shape[0]).to(s.device)
                return s.mean()

            loss_scl = get_self_cl(outputs['hr_vector'])
            loss_scl += get_self_cl(outputs['tail_vector'])
            loss_scl = self.args.self_cl_weight * loss_scl

            loss_extra = loss_scl

            # 加一个头提取任务
            # loss_htr = torch.norm(outputs['tail_vector'])
            ### end hmt
            ...
            if i % self.args.print_freq == 0:
                ...
                if self.wandb_logger:
                    ...
                    for m in ms:
                        m = m.__dict__
                        log_dict[f"train_{m['name']}"] = float(m['val'])
                    self.wandb_logger.log(log_dict)
